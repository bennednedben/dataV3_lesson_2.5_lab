...
-- These three actors have the same last name. So we do not want to include this last name in our output. Last name "Astaire" is present only one time with actor 
...
-- 2
-- Which last names appear more than once? We would use the same logic as in the previous question but this time we want to include the last 
...
SELECT staff_id, COUNT(*) as 'rentals/employee'
FROM sakila.rental
GROUP BY staff_id;
...
FROM sakila.film
GROUP BY rating
HAVING 'average_length' > 120; -- ???? don't work...why????
